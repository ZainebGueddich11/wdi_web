...
class FlaskIntegrationService
{
    ...
    public function executeFlaskAPIEnvironement(array $data): array
{
    $httpClient = HttpClient::create();

    $response = $httpClient->request(
        'POST',
        $this->flaskEndpoint.'/predictEnvironement', // Replace with the appropriate endpoint on your Flask application
        [
            'json' => $data,
        ]
    );
